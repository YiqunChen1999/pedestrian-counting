...
class ATSSHead_dcd(AnchorHead):
    ...
    def __init__(self,
        ...
        super(ATSSHead_dcd, self).__init__(
            num_classes,
            in_channels,
    ...
    def _init_layers(self):
        ...
        for i in range(self.stacked_convs):  # 4
            ...
            self.cls_convs.append(
                nn.Sequential(
                    conv_dy(chn, self.feat_channels, kernel_size=3, stride=1, padding=1, bias=False, K=self.K, t=self.t),
        ...
        self.atss_centerness = nn.Conv2d(
            ...
            padding=pred_pad_size)
        '''
        self.atss_cls = conv_dy(self.feat_channels, self.num_anchors * self.cls_out_channels, kernel_size=self.pred_kernel_size,
                                stride=1, padding=pred_pad_size)
        self.atss_reg = conv_dy(self.feat_channels, self.num_base_priors * 4, kernel_size=self.pred_kernel_size,
                                stride=1, padding=pred_pad_size)
        self.atss_centerness = conv_dy(self.feat_channels, self.num_base_priors * 1, kernel_size=self.pred_kernel_size,
                                stride=1, padding=pred_pad_size)
        '''
